...
public class AnalyticsDashboardSystem : MonoBehaviour
{
    ...
    [SerializeField]
    private FitnessTrackingSystem fitnessSystem;
    ...
    private void Start()
    {
        ...
        if (fitnessSystem == null)
            fitnessSystem = FindFirstObjectByType<FitnessTrackingSystem>();
        if (inputSystem == null)
        ...
        if (mainMenuBtn)
            mainMenuBtn.onClick.AddListener(() => gameManager?.ReturnToIdle());
        if (gameManager != null)
    ...
    private void Populate()
    {
        ...
        {
            durationText.text =
                $"Duration: {System.TimeSpan.FromSeconds(fitnessSystem.Duration):mm\\:ss}";
            calText.text = $"Calories: {fitnessSystem.Calories:F1}";
        }
        if (inputSystem)
            accuracyText.text = $"Accuracy: {inputSystem.AccuracyPercentage:F0}%";
        if (scoringSystem)
        {
